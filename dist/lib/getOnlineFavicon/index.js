...
// URL先にファビコンが存在するか
const existsFavicon = async (url) => {
    const res = await fetch(url);
    if (!res.ok)
        throw new Error('res.ok == false');
    if (!isIcon(res))
        throw new Error('content-type is not icon or image');
    return url;
};
// 画像URLから幅、高さを取得
...
const getFaviconFromGoogleAPI = (url) => 'https://www.google.com/s2/favicons?domain=' + url.origin;
// URLからルートにあるファビコン https://example.com/favicon.ico を取得
const getFaviconFromRoot = async (url, maxSize = Infinity) => {
    const resUrl = await existsFavicon(url.origin + '/favicon.ico');
    const size = (await getImageDimensions(resUrl)).width;
    if (maxSize < size)
        throw new Error('最大許容サイズを満たしませんでした。画像: ' + size + 'px, 指定された最大許容サイズ: ' + maxSize + 'px');
    return resUrl;
};
// head要素内から複数のファビコンを取得（画像のサイズが大きい順にソート）
const getFaviconsFromHead = async (url, d) => {
    return (await Promise.all([...d.head.querySelectorAll('link[rel*=icon]')].map(async (link) => {
        const sizesValue = link.getAttribute('sizes');
        const hrefValue = link.getAttribute('href');
        let href, size = sizesValue ? parseFloat(sizesValue) : null, exists = null;
        try {
...
const getFaviconFromPage = async (url, maxSize = Infinity, opt) => {
    ...
    for (const { exists, href, size } of iconDatas) {
        ...
        if (isRootFavicon)
            existsRootFavicon = exists;
        if (exists === false || (typeof size == 'number' && maxSize < size))
            continue;
        if (!exists) {
            ...
            if (isRootFavicon)
                existsRootFavicon = existsResult;
            if (!existsResult)
                continue;
        }
        resultUrl = href;
...
export const getAll = async (url, opt = {}) => {
    // headから
    let favList = [], existsRootFav = false;
    try {
